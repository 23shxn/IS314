...
import { Calendar, Car, MapPin, Clock, CheckCircle, X, Eye, LogOut, Layout, Users, ClipboardList, ToolCase, UserPlus } from 'lucide-react';
import axios from 'axios';
import '../styles/SuperAdminDashboard.css';

const ReservationManagement = ({ reservations, setReservations, currentUser, setCurrentUser }) => {
  ...
  const userReservations = currentUser?.role === 'SUPER_ADMIN'
    ? reservations.filter(res => {
        const today = new Date();
        const rentalStart = new Date(res.rentalDate);
        const rentalEnd = new Date(res.returnDate);
        return res.status.toLowerCase() === 'confirmed' && today >= rentalStart && today <= rentalEnd;
      })
    : reservations.filter(res => currentUser && res.userId === currentUser.id);
